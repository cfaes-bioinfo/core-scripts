...
# Defaults - workflow
workflow_version=3.0.1                                      # The version of the assemblyqc pipeline
work_dir=/fs/scratch/"$osc_account"/$USER/assemblyqc         # 'work dir' for initial outputs (selected, final outputs go to the outdir)
...
USAGE / EXAMPLE COMMANDS:
  - Basic usage example:
      sbatch $0 -i assemblysheet.csv -p params.yml -o results/asmqc
  - With an extra config file and a specific pipeline version:
      sbatch $0 -i assemblysheet.csv -p params.yml -o results/asmqc --config extra.config --workflow_version $workflow_version

REQUIRED OPTIONS:
  -i/--samplesheet    <file>  Assemblysheet (CSV) with the assemblies to QC, see:
                              https://github.com/Plant-Food-Research-Open/assemblyqc/blob/main/docs/usage.md#assemblysheet-input
  -p/--params         <file>  YAML file with workflow parameters, see:
                              https://github.com/Plant-Food-Research-Open/assemblyqc/blob/main/docs/parameters.md
...
OUTPUT:
  Alongside the pipeline's own output, '<outdir>/logs' will contain:
    ...
    versions.txt    - Versions of this script and of Nextflow
...
UTILITY OPTIONS:
  ...
  -v/--version                Print script and Nextflow versions
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -i | --samplesheet )        check_val "$1" "${2:-}"; shift; samplesheet=$1 ;;
...
[[ -z "$samplesheet" ]] && die "No samplesheet specified, do so with -i/--samplesheet" "$all_opts"
...
echo "Pipeline version:                         $workflow_version"
...
ls -lh "$samplesheet" "$params_file"
set_threads "$IS_SLURM"
[[ "$IS_SLURM" == true ]] && slurm_resources
echo "=========================================================================="
log_time "Printing the contents of the samplesheet:"
cat -n "$samplesheet"
